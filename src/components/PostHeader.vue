<template>
  <header>
    ...
    >
      <div
        class="container max-w-lg text-center px-6 absolute z-10"
        :class="[post.fullscreen ? 'flex flex-col items-center m-auto pin': 'mx-auto pin-b pin-x pb-16']"
      >
        <div class="m-auto">
          <p class="text-white text-xs mb-2 uppercase">{{ post.timeToRead }} min read</p>
          <h1 class="text-white sm:text-4xl font-sans font-bold mb-2">{{ post.title }}</h1>
          <p class="text-white">
      ...
      <div class="container max-w-lg mx-auto text-center px-6">
        ...
        <p class="text-grey-dark">
          <span v-if="post.author">
            <g-link
              :to="`${post.author.path}/`"
              class="text-grey-dark no-underline capitalize hover:border-b"
            >{{ titleCase(post.author.title) }}</g-link>&bull;
          </span>
          <time :datetime="post.datetime" class="capitalize">{{ formattedPublishDate }}</time>
        </p>
